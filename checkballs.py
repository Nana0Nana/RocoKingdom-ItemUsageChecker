...
def prompt_resume_from_cache(work_dir: Path):
    """Check local cache and ask if user wants to resume from a specific page.
    Returns (start_page_1indexed, resume_mode) or (None, False)."""
    json_file = work_dir / OUTPUT_FILE
    ...
    has_next = str(data.get("has_next", "?"))
    skipped = data.get("skipped_pages", []) or []

    print()
    print("========== 检测到本地缓存 ==========")
    print(f"当前缓存: {count} 条数据")
    print(f"上次拉取到: 第 {page} 页")
    print(f"是否还有下一页: {has_next} (1=有 0=无 ?=未知)")
    if skipped:
        preview = ", ".join(str(p) for p in skipped[:10])
        more = f" ...(共{len(skipped)}页)" if len(skipped) > 10 else ""
        print(f"上次被跳过的页: {preview}{more}")
    print("====================================")

    if has_next == "0":
        print("[+] 上次拉取已完整, 本次走增量模式 (仅补拉更新数据)")
        return None, False

    print()
    print("上次拉取未完成, 可以断点续传继续拉取旧数据:")
    print(f"  - 直接回车: 跳过断点续传, 走增量模式")
    print(f"  - 输入页码: 从该页开始继续 (建议输入 {page + 1} 从下一页继续)")

    try:
        user_input = input("起始页码> ").strip()
    ...
    try:
        start_page = int(user_input)
    except ValueError:
        print(f"[警告] 无效的页码 {user_input!r}, 跳过断点续传")
        return None, False

    if start_page < 1:
        print(f"[警告] 页码必须 >= 1, 跳过断点续传")
        return None, False
...
def wait_for_child_exit(proc: subprocess.Popen):
    while True:
...
def parent_main():
    ...
    try:
        ...
        print("[+] 获取完成后会自动关闭代理并退出，期间也可以按 Ctrl+C 手动结束")
        wait_for_child_exit(proc)
        print("[+] 正在恢复系统代理...")
        restore_proxy_state(original_state)
        original_state = read_proxy_state()
        print("[+] 系统代理已恢复，结果页会继续保留")
        print("[+] 按 Enter 退出程序，或直接关闭窗口")
        try:
            input()
...
def parent_main_v2():
    ...
    resume_start_page, resume_mode = prompt_resume_from_cache(work_dir)
    if resume_mode:
        os.environ[START_PAGE_ENV] = str(resume_start_page)
        os.environ[RESUME_MODE_ENV] = "1"
        print(f"[+] 断点续传: 将从第 {resume_start_page} 页开始继续拉取")
    else:
        os.environ.pop(START_PAGE_ENV, None)
        os.environ.pop(RESUME_MODE_ENV, None)
